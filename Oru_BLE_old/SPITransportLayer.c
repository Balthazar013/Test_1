	...
	case (SPIHDLR_TXRX):
		...
		if(hasNewByte == TRUE)
		{
			if(SPIChar[0] == PKT_SOF)
			{
				...
				for (TransportLayer_p = 0; TransportLayer_p < 41; TransportLayer_p++) 
				{				
...
/****************************************STATE SPIHDLR_WAIT_ACK********************************************************/		
	case (SPIHDLR_WAIT_ACK):
		...
		break;
/****************************************STATE SPIHDLR_PUSH_NACK********************************************************/
	case (SPIHDLR_PUSH_NACK):

		tmp = PKT_T_NACK;
		tmp |= (uint8_t) (((attr.rxBuffer[0] & PKT_COUNTER_MASK) >> 4) << 4);
	...
	break;
/****************************************STATE SPIHDLR_RETX_ACK********************************************************/
	case (SPIHDLR_RETX_ACK):
