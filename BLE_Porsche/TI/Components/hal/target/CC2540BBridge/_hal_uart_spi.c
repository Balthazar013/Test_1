...
#define SPI_MAX_PKT_LEN            80
#define SPI_MAX_DAT_LEN           (SPI_MAX_PKT_LEN - SPI_FRM_LEN) /* 76 = 3xBytes_HDR + 18Bytes*4xPackets_DATA + 1xByte FrameControl*/
...
static volatile uint8 readActive = 0;
...
static void HalUARTPollSPI(void)
{
  ...
  pktFound = FALSE;
#endif
  if ( ( spiRdyIsr ) /*|| (SPI_RDY_IN())*/ ) //SPI_RDY_IN it set to 0  by the panther once it goes to sleep mode :it is an input value 
  {
    CLEAR_SLEEP_MODE();
...
#if defined POWER_SAVING
  if  ( /*SPI_RDY_IN() ||*/ SPI_RX_RDY() || spiRxLen || spiTxLen || spiRdyIsr ||  pktFound || SPI_RDY_OUT() )  //SPI_RDY_IN it set to 0  by the panther once it goes to sleep mode :it is an input value .
  {
    CLEAR_SLEEP_MODE();
  }
...
{
#else
#if  ((HAL_UART_SPI == 1) || defined HAL_SPI_MASTER || (HAL_UART_SPI == 4))
HAL_ISR_FUNCTION(port0Isr, P0INT_VECTOR)
#elif (HAL_UART_SPI == 2) || (HAL_UART_SPI == 3)
