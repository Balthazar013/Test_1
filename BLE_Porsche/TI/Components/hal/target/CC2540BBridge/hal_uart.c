...
static halUARTCBackTxComplete_t txCompleteCB;
...
void HalUART_SetTxCompleteCB(halUARTCBackTxComplete_t aCB){ /* Custom add by PES for the BBridge Project */
  txCompleteCB = aCB;
}
...
#endif
#if ((HAL_UART_SPI == 1) || (HAL_UART_SPI == 4))
  if (port == HAL_UART_PORT_0)  HalUARTOpenSPI(config);
#endif
#if (HAL_UART_SPI == 2) || (HAL_UART_SPI == 3)
