...
void halSleep( uint32 osal_timeout )
{
  ...
  if ( (timeout > PM_MIN_SLEEP_TIME) || (timeout == 0) )
  {
	...
	//  P2DIR = 0x00; //All inputs 
	 /* Disable Pull-ups */
      P2INP = 0xFF; //De-select Pull-Up for all Input Pins
	  P1INP = 0xFF; //De-select Pull-Up for all Input Pins
	  P0INP = 0xFF;//0xF6; //De-select Pull-Up for all Input Pins

      /* Set Low Power Mode PM1 0x01, PM2 0x02,  PM3 0x03 */   
     X_SLEEPCMD = 0x03;
